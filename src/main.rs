...
use iced::{
    Color, Element, Fill, Subscription, Task, Theme, keyboard,
...
impl AppState {
    fn view(&self) -> Element<Message> {
        ...
        let header = row![
            text(self.current_dir.to_str().unwrap())
                .size(24)
                .width(Fill),
        ...
        for val in &self.current_files {
            ...
            current_column = current_column.push(text(val.0.clone()).size(24).color(color));
            count += 1;
        ...
        let file_columns = row(columns
            .into_iter()
            .map(|col| col.into())
            .collect::<Vec<_>>())
        .spacing(16);

        column![header.width(Fill), horizontal_rule(2), file_columns]
            .spacing(16)
            .into()
    }
    ...
    fn update(&mut self, msg: Message) -> Task<Message> {
        match msg {
            ...
            Message::Enter => {
                let path = path::absolute(PathBuf::from(
                    self.current_files[self.selected_file as usize].0.clone(),
                ))
                .unwrap();
